...
def log_stop_information(partridge_feed, stop_id):
    ...
    logger.debug('stop_coordinates: {:.3f}, {:.3f}'.format(stop['geometry'].x, stop['geometry'].y))
    stop_times = partridge_feed.stop_times
    trip_ids = stop_times[stop_times['stop_id'] == stop_id]['trip_id'].unique()
    route_ids = partridge_feed.trips[partridge_feed.trips['trip_id'].isin(trip_ids)]['route_id'].unique()
    routes = partridge_feed.routes[partridge_feed.routes['route_id'].isin(route_ids)]
    logger.debug('Routes:')
    for route_id, short_name, long_name in routes[['route_id', 'route_short_name', 'route_long_name']].itertuples(index=False, name=None):
